...
-- SplAssignment x = 1
eval ((SplAssignment (name) (SplInt n)), rest,e ) 
    | get3rd (getVarTuple name e) == TyInt =  (head rest, tail rest, newEnv:(deleteEnvir name e))
    | otherwise = error "type do not match"
    where newEnv = (changeValue (show n) (getVarTuple name e))

-- SplAssignment x = Bool
eval ((SplAssignment (name) (SplFalse)), rest,e ) 
    | get3rd (getVarTuple name e) == TyBool =  (head rest, tail rest, newEnv:(deleteEnvir name e))
    where newEnv = (changeValue ("False") (getVarTuple name e))

-- SplAssignment x = Bool
eval ((SplAssignment (name) (SplTrue)), rest,e ) 
    | get3rd (getVarTuple name e) == TyBool =  (head rest, tail rest, newEnv:(deleteEnvir name e))
...
evalExpr (SplVar name) e = (SplInt newValue)
    where newValue = (read (get2nd (getVarTuple name e)))

evalExpr (SplIntListgetElement name exp) e = (SplInt newValue)
    where newValue = (getElement (getNum (evalExpr (exp) e)) (read (get2nd (getVarTuple name e))::[Int] ))
...
changeValue :: String -> (String,String,SplType) -> (String,String,SplType)
changeValue newValue varTuple = (get1st varTuple,newValue,get3rd varTuple)

getElement :: Int -> [Int] -> Int
getElement 0 (x:xs) = x
getElement n (x:xs) = getElement (n-1) xs
getElement _ [] = error "out of list limit" 
